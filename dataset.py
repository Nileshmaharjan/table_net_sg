...
class ImageFolder(nn.Module):
    ...
    def __getitem__(self, index):
        ...
        image2 = np.array(Image.open(img_path))
        image3 = np.array(Image.open(img_path))
        image_stack = np.stack([image1, image2, image3], axis=2)

        table_image = torch.FloatTensor(np.array(Image.open( table_mask_path))/255).reshape(1, 1024, 1024)
        ...
        image = self.transform(image=image_stack)['image']
        return {"image": image, "table_image": table_image}
